...
CollectorClient::CollectorClient(std::string broker_addr, std::string auth_token)
    : broker_addr_(broker_addr), auth_token_(auth_token), connected_(false) {
...
::grpc::Status CollectorClient::RegisterDatapoints(
    ::grpc::ClientContext* context, const ::sdv::databroker::v1::RegisterDatapointsRequest& request,
    ::sdv::databroker::v1::RegisterDatapointsReply* response) {
    return stub_->RegisterDatapoints(context, request, response);
}

::grpc::Status CollectorClient::UpdateDatapoints(
    ::grpc::ClientContext* context, const ::sdv::databroker::v1::UpdateDatapointsRequest& request,
...
std::unique_ptr<grpc::ClientContext> CollectorClient::createClientContext() {
    ...
    for (const auto& metadata : metadata_) {
        ...
        LOG_TRACE << "  AddMetadata(" << metadata.first << ", " << metadata.second << ")"
                  << std::endl;
    }
