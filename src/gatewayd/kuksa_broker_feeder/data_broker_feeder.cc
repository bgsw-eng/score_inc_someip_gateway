...
class DataBrokerFeederImpl final : public DataBrokerFeeder {
   private:
   ...
   public:
    DataBrokerFeederImpl(std::shared_ptr<CollectorClient> client, DatapointConfiguration&& dp_config)
        : client_(client), dp_config_(std::move(dp_config)), feeder_active_(true) {
    ...
    void Run() override {
        while (feeder_active_) {
            LOG_INFO << "Connecting to data broker [" << client_->GetBrokerAddr() << "] ..."
                     << std::endl;
    ...
    bool registerDatapoints() {
        ...
        if (status.ok()) {
            ...
            for (const auto& name_to_id : id_map_) {
                LOG_DEBUG << "    '" << name_to_id.first << "' -> " << std::dec << name_to_id.second
                          << std::endl;
            }
            return true;
        }
        ...
        for (const auto& value : values_to_feed) {
            ...
            if (iter != id_map_.end()) {
                ...
                LOG_DEBUG << "    '" << value.first << "' (" << id << ") of type "
                          << std::dec << (int)value.second.value_case() << ", value: { "
                          << value.second.ShortDebugString() << " }" << std::endl;
            } else {
...
std::shared_ptr<DataBrokerFeeder> DataBrokerFeeder::createInstance(
    std::shared_ptr<CollectorClient> client, DatapointConfiguration&& dpConfig) {
